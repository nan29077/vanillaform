...
const SLUG_META: Record<string, { label: string; iconName: string }> = {
  "seller-guide": { label: "라이브 셀러 신청 안내", iconName: "Inquiry" },
  contact:        { label: "1대1 문의",             iconName: "Message" },
  faq:            { label: "자주 묻는 질문",         iconName: "Help" },
  shipping:       { label: "배송 안내",              iconName: "Shipping" },
...
export default function FooterContentManager() {
  ...
  async function handleAdd() {
    ...
    if (!slug || !adding.title.trim()) { setAddError("슬러그와 제목은 필수입니다."); return; }
    if (!/^[a-z0-9-]+$/.test(slug)) { setAddError("슬러그는 영문 소문자, 숫자, 하이픈(-)만 사용할 수 있습니다."); return; }
    if (items.some((it) => it.slug === slug)) { setAddError("이미 존재하는 슬러그입니다."); return; }
    setSaving(true);
    try {
      const res = await fetch("/api/admin/footer-content", {
        method: "PUT",
      ...
      if (!res.ok) { setAddError("저장에 실패했습니다."); return; }
      await load();
      setAdding(null);
      setShowSavedPopup(true);
    } finally {
    ...
    <div className="bg-white rounded-xl border border-amber-200 p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-amber-700 flex items-center gap-2">
          <Icon name="File" size={14} /> 새 콘텐츠 추가
        </h2>
      ...
      <div className="flex gap-2 justify-end">
        <button onClick={() => setAdding(null)} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg">취소</button>
        <button onClick={handleAdd} disabled={saving} className="px-4 py-2 text-sm font-bold bg-amber-500 text-white rounded-lg hover:bg-amber-600 disabled:opacity-50 flex items-center gap-1.5">
  ...
  if (editing) return (
    <div className="bg-white rounded-xl border border-amber-200 p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-amber-700 flex items-center gap-2">
          <Icon name={SLUG_META[editing.slug]?.iconName ?? "File"} size={14} />
          {editing.title} 편집
        </h2>
      ...
      <div className="flex gap-2 justify-end">
        <button onClick={() => setEditing(null)} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg">취소</button>
        <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm font-bold bg-amber-500 text-white rounded-lg hover:bg-amber-600 disabled:opacity-50 flex items-center gap-1.5">
          <Icon name="Check" size={14} />
          {saving ? "저장 중..." : "저장"}
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs text-gray-400">지원 페이지(이용약관, FAQ, 배송안내 등)의 내용을 관리합니다.</p>
        <div className="flex items-center gap-2">
          <button onClick={load} className="p-2 text-gray-400 hover:text-gray-700">
            <Icon name="Reorder" size={16} />
          </button>
          <button
            onClick={() => { setAddError(""); setAdding({ slug: "", title: "", content: "" }); }}
